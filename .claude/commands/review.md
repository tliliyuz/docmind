...
- [ ] **循环导入**：是否存在模块间循环依赖（A → B → A）？检查 `from app.xxx` 导入链是否形成环；循环导入视为 🔴 严重问题
- [ ] **跨层耦合**：api/ 层是否直接访问数据库/文件系统（绕过 service 层）？service 层是否直接操作 HTTP 请求/响应对象（绕过 api 层）？跨层耦合视为 🔴 严重问题
- [ ] **紧耦合接口**：类/模块间是否通过具体类而非接口/协议通信（如函数签名要求 `ChromaVectorStore` 而非 `VectorStore` 抽象基类）？紧耦合视为 🟡 规范问题——接口变更时级联修改范围大
- [ ] **上帝对象/模块**：是否存在单文件/单类承载过多职责（如一个 service.py 包含 KB 管理 + 文档处理 + 权限校验 + 通知发送）？职责过多视为 🟡 规范问题——修改任一维度都可能意外影响其他维度
- [ ] **链式调用穿透**：是否存在 `a.b.c.d()` 形式的深层链式调用（Law of Demeter 违反）？链式调用 ≥3 层视为 🟡 规范问题——中间对象变更时调用方全部断裂
- [ ] **隐式副作用**：函数是否在返回值之外修改了全局状态、模块级变量、或调用方未预期的外部资源？隐式副作用视为 🟡 规范问题——调用方无法从签名预判行为
- [ ] **硬编码依赖**：是否通过 `import` 直接绑定具体实现（如 `import redis` 而非依赖注入），导致替换实现需要修改业务代码？硬编码依赖视为 🟡 规范问题

##### 代码重复检查

- [ ] **复制粘贴代码块**：是否存在 ≥6 行的重复代码块（含变量名仅改前缀/后缀的机械复制）？视为 🔴 严重问题——修改时必然遗漏某一处
- [ ] **相似逻辑分散**：是否存在多个函数实现了相同/高度相似的业务规则（如「判断用户是否有 KB 访问权限」逻辑在 3+ 处独立实现）？视为 🔴 严重问题——规则变更时需要逐处寻找修改
- [ ] **重复的条件分支**：是否存在相同的 `if/else` 或 `match/case` 分支结构在 2+ 处出现（如「根据 doc_type 选择处理策略」的分发逻辑重复）？视为 🟡 规范问题——新增类型时需修改多处
- [ ] **重复的错误处理样板**：是否存在相同的 `try/except` + 日志 + 异常转换模式在 3+ 处出现？视为 🟡 规范问题——应提取为上下文管理器或装饰器
- [ ] **重复的校验逻辑**：Pydantic schema 之间是否存在重复的 `@validator`/`@field_validator` 逻辑？API 路由之间是否存在重复的参数校验步骤？视为 🟡 规范问题——应提取为可复用 validator 或依赖项
- [ ] **重复的常量/字面量**：相同的魔法数字、字符串常量、配置值是否在 3+ 处独立定义（而非提取为模块级常量或 `config.py` 配置项）？视为 🟡 规范问题
- [ ] **重复的 SQL 查询模式**：是否存在相同的查询条件组合在多个 service 函数中独立拼装（如「未删除 + 属于某 KB + 状态为 READY」的条件堆叠）？视为 🟡 规范问题——应提取为查询辅助函数或 repository 方法
...
**耦合度：**
- [ ] 是否存在循环导入（A → B → A）
- [ ] api/ 层是否直接访问数据库/文件系统（跨层耦合）
- [ ] 是否存在通过具体类而非接口通信（紧耦合接口）
- [ ] 是否存在上帝对象/模块（单文件承载过多职责）
- [ ] 是否存在深层链式调用（a.b.c.d()，≥3 层）
- [ ] 函数是否存在隐式副作用（修改全局状态/外部资源）
- [ ] 是否存在硬编码依赖（直接 import 具体实现而非依赖注入）

**代码重复：**
- [ ] 是否存在 ≥6 行复制粘贴代码块
- [ ] 是否存在相同业务规则在 3+ 处独立实现
- [ ] 是否存在相同条件分支结构在 2+ 处重复
- [ ] 是否存在相同 try/except 样板在 3+ 处出现
- [ ] 是否存在重复的 Pydantic validator / 路由校验逻辑
- [ ] 是否存在相同常量/魔法数字在 3+ 处独立定义
- [ ] 是否存在相同 SQL 查询条件在多个函数中独立拼装
