...
│  │           CoarseRank→Rerank→Evidence→Prompt）→ LLM SSE │   │
│  └────────────────────────┬───────────────────────────────┘   │
│                           │                                   │
│  ┌──────────┬─────────────┼──────────────┬────────────────┐  │
...
│     ↓                                                                    │
│ [CoarseRank] 粗排 → 向量相似度过滤低分 chunk，缩候选池（ADR-024）              │
│     ↓                                                                    │
...
| CoarseRank 粗排 | 粗排异常 / 过滤后候选不足 RERANK_TOP_K → 跳过精排直接返回候选 |
...
**粗排（Coarse Ranking）— ADR-024 新增**：RRF 融合后在 Rerank 精排前插入向量相似度粗排。复用 query embedding 与每条候选 chunk embedding 做余弦相似度计算（L2 归一化后点积，≤20 次，<1ms），过滤低分 chunk（阈值 `COARSE_RANK_THRESHOLD=0.3`）并通过上限截断（`COARSE_TOP_K=15`），将候选池从 ≤20 缩至 ~15 再送精排。粗排失败或过滤后候选不足 `RERANK_TOP_K` 时降级跳过精排。

**精排（Rerank）— DashScope Rerank API（qwen3-rerank）**：中文场景首选，有免费额度。对粗排后的候选池做语义精排，按 relevance_score 降序排列。API 异常时降级回退到原始 RRF 排序（`retrieval_output.results[:top_k]`），不阻断检索管线。

> **架构决策**：[ADR-024](decisions/ADR-024-粗排层.md)（2026-06-23 采纳）
