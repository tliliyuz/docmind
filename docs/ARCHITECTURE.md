...
| 搜出来的结果排序不准 | **Rerank 重排序** | 当前 NoopReranker 占位，后续 DashScope Rerank 精排 | 相关文档排在前面 |
...
│     ↓                                                    │
│ [Rerank] 重排序 → NoopReranker 占位，后续接入 DashScope    │
│     ↓                                                    │
│ [Evidence] 句级 BM25 定位 → 每个 chunk 内选出最佳证据句     │
│     ↓                                                    │
│ [Prompt] 组装 Prompt → 拼接检索结果 + 历史消息 + 用户问题   │
└──────────────────────────────────────────────────────────┘
    ↓
...
| Prompt 组装 | 软上限 + 相关性优先填充，Token 预算四池子分拆（见 §8 会话记忆） | [RAG_PIPELINE.md §3](../backend/docs/RAG_PIPELINE.md) |
| SSE 事件流 | 6 事件类型（meta/thinking/message/sources/finish/error）+ 15s 心跳 | [RAG_PIPELINE.md §5](../backend/docs/RAG_PIPELINE.md) |
...
| Evidence Highlight | 检索时句级 BM25 选句，解耦 LLM 引用格式，确定性定位 | [RAG_PIPELINE.md §7](../backend/docs/RAG_PIPELINE.md) |
...
| 路径 | 技术 | 关键参数 |
|:---|:---|:---|
| 向量检索 | BaseVectorStore/ChromaDB + DashScope text-embedding-v3 | 1024 维 / cosine / top_k=10 / `where={"kb_id": kb_id}` |
| BM25 关键词 | rank-bm25 (BM25Okapi) + jieba 分词 | 三级缓存（进程内 60s / Redis 300s / MySQL 懒加载），详见 [RAG_PIPELINE.md §2.2](../backend/docs/RAG_PIPELINE.md) |
...
> **设计决策**：[ADR-018](../docs/decisions/ADR-018-向量存储抽象层.md)（2026-06-15 采纳）
...
**Collection 策略**：当前仍为**共用 Collection + Metadata 隔离**（`docmind` collection，`kb_id` metadata 过滤）。独立 Collection 的判断标准：3 个以上完全独立业务线。

**metadata 类型一致性**：ChromaDB 对 metadata 值的类型敏感（整数 `1` 和字符串 `"1"` 视为不同值）。强制约定 metadata 值统一使用原生 int（`kb_id`/`doc_id`/`chunk_index`），入库和查询两端直接传 int，禁止字符串化。

**KB 删除（单 Collection 架构）**：`vector_store.delete(where={"kb_id": kb_id})` 通过 metadata filter 批量删除向量，而非 drop collection。

**持久化与初始化**：`PersistentClient`，数据持久化到 `CHROMA_PERSIST_DIR`（默认 `./chroma_data`）。采用**懒加载自动初始化**——`get_vector_store()` 首次调用时自动初始化 PersistentClient 并获取/创建 collection，确保 FastAPI（lifespan）和 Celery Worker（独立进程）两种运行时都能正确初始化。索引 `hnsw:space=cosine`（余弦相似度）。
...
| 方案 | 说明 |
|:---|:---|
| **NoopReranker（当前）** | 保持 RRF 排序（相关性降序），截取 top_k=5，不按长度重排 |
| **DashScope Rerank API（扩展）** | 中文场景首选，有免费额度 |
...
**共享权限函数**：`app/core/permissions.py` 提供三个纯函数，所有接口和 service 层统一使用：
- `require_kb_readable(kb, user_id, role)` — visibility 优先（public→所有用户，private→owner+admin）
- `require_kb_writable(kb, user_id, role)` — ownership 基础（owner+admin 可删/改元数据）
- `require_kb_owner(kb, user_id)` — owner-only（上传/reprocess，admin 不可代传）
...
system prompt → 历史消息 → 当前 question。`_load_history()` 从 DB 查询最近 40 条消息，反转为时间正序，从旧到新累加 token 直到 `HISTORY_BUDGET`。
...
**闲谈路径同样注入历史消息**：`_load_history()` 在意图判断之前执行，保证对话连续性。闲谈用 `CASUAL_SYSTEM_PROMPT`（定义于 `app/rag/knowledge_pipeline.py`）+ 历史 + 当前问题，跳过检索但保留上下文。

### 8.10 问题重写

设计细节见 [RAG_PIPELINE.md §4](../backend/docs/RAG_PIPELINE.md)。要点：轻量 `_needs_rewrite()` 触发判断（正常路径零额外延迟），仅取最近 2 轮（4 条消息），LLM 失败降级到原始 question。
...
| 局限 | 说明 | 缓解 |
|:-----|:-----|:-----|
| 向量存储规模上限 | 共用 Collection + Metadata 隔离适用于 < 5 万 chunk 总量 | total_chunks > 5 万或 P95 > 500ms 时评估独立 Collection；BaseVectorStore 抽象层（ADR-018）已支持未来切换向量库 |
