...
[Fusion] RRF 融合排序 → 合并两路结果
    ↓
[Rerank] NoopReranker → 按 chunk 长度升序排列 + 截取 top_k=5
    ↓
...
| 检索后排序 | 按 chunk 长度升序排列（短者优先） | 短 chunk 信息密度高，相同预算下覆盖更多文档 |
| Prompt 组装 | 软上限 + 择优填充 | 超预算时尝试下一个更短 chunk，而非直接 break |
...
async def chat(question, conversation_id, kb_id, deep_thinking, db, current_user):
    ...
    # 4. 重排序 — [Phase 3] NoopReranker：按长度排序 + 截取
...
| 阶段 | 方案 | 说明 |
|:---|:---|:---|
| Phase 3 | **NoopReranker（占位）** | 按 chunk 长度升序排列（短 chunk 优先，信息密度高），截取 top_k=5 |
