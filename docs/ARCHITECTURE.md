...
| 限流 | 固定窗口计数器 + Redis | IP 级频率限制，4 接口组独立阈值，详见 §13.2 | [Implemented] |
...
| 可视化图表 | ECharts 5 | Admin 统计页图表（趋势/延迟/Token），从 traces 表聚合 | [Implemented] |
...
**决策**：外部暴露的资源（knowledge_bases、documents、conversations）采用双字段方案。

**背景**：原始设计中所有表使用 `BIGINT AUTO_INCREMENT` 作为主键，ID 直接暴露在 URL 和 API 响应中，存在 ID 枚举风险。
...
| 资源 | 决策 | 原因 |
|:---|:---|:---|
| Knowledge Base | ✅ 改造 | 知识库天然是共享资源，未来协作/公开场景必须安全 |
| Document | ✅ 改造 | 嵌套路径暴露内部编号，未来下载/分享链接需要 |
| Conversation | ✅ 改造 | 聊天链接直接暴露，用户可枚举；SSE 统一 |
| Trace | ✅ 移除自增 id | 已有 UUID（trace_id），响应中暴露自增 id 纯属多余 |
| User | ⚠️ 暂缓 | Admin 接口仅开发者使用，JWT 已有保护，收益极低 |
| Message | ❌ 不改造 | 仅 SSE 返回，用户不会操作；数据量大，改造代价高 |
| Chunk | ❌ 不改造 | 检索内部结构，无独立接口，不是资源 |
...
**实现要点**：
- 外键列保持 BIGINT 不变，仅在 API 边界通过 JOIN 转换为 uuid
- 请求体中的 `kb_id` / `conversation_id` 改为接收 UUID 字符串
- SSE `meta` 事件的 `conversation_id` 返回 uuid；`finish` 事件的 `message_id` 保持 int（不改造 Message）
- 每个 service 新增 UUID 解析辅助函数（如 `_get_kb_by_uuid()`），内部查询仍用整数 id
- `conversations.original_kb_uuid`：KB 物理删除前由 Celery 同步备份 `kb.uuid`，与 `original_kb_id` / `original_kb_name` 一起批量写入，用于孤儿会话审计追踪
...
┌─────────────────────────────────────────────────────────────┐
│                      Nginx (port 80)                          │
│  反向代理 + 静态资源托管（SSL 终结 [Planned: Phase 6]）         │
...
```
backend/app/middleware/rate_limit_middleware.py   ← 新建：RateLimitMiddleware（纯 ASGI middleware）
backend/app/config.py                            ← 新增 6 个配置项
backend/app/main.py                              ← app.add_middleware(RateLimitMiddleware)
backend/app/core/redis_client.py                 ← 修改：ThreadedRedisClient 新增 incr/expire/eval 方法
backend/tests/test_rate_limit.py                 ← 新建：22 用例（A8.1-A8.5 + 单元测试）
```
