...
| 阶段 | 核心决策 | 详见 |
|:-----|:---------|:-----|
| 多路检索 | 向量（ChromaDB 1024 维）+ BM25（rank-bm25 + jieba）双路检索，三级缓存（进程内/Redis/MySQL） | [RAG_PIPELINE.md §2](../backend/docs/RAG_PIPELINE.md) |
...
| 路径 | 技术 | 关键参数 |
|:---|:---|:---|
| 向量检索 | ChromaDB + DashScope text-embedding-v3 | 1024 维 / cosine / top_k=10 / `where={"kb_id": kb_id}` |
...
当前：**共用 Collection + Metadata 隔离**（`docmind` collection，`kb_id` metadata 过滤）。独立 Collection 的判断标准：3 个以上完全独立业务线。
...
**KB 删除（单 Collection 架构）**：`collection.delete(where={"kb_id": kb_id})` 通过 metadata filter 批量删除向量，而非 drop collection。

**持久化与初始化**：`PersistentClient`，数据持久化到 `CHROMA_PERSIST_DIR`（默认 `./chroma_data`）。采用**懒加载自动初始化**——`get_collection()` 首次调用时自动初始化 PersistentClient 并获取/创建 collection，确保 FastAPI（lifespan）和 Celery Worker（独立进程）两种运行时都能正确初始化。索引 `hnsw:space=cosine`（余弦相似度）。
...
**闲谈路径同样注入历史消息**：`_load_history()` 在意图判断之前执行，保证对话连续性。闲谈用 `CASUAL_SYSTEM_PROMPT` + 历史 + 当前问题，跳过检索但保留上下文。
...
| 局限 | 说明 | 缓解 |
|:-----|:-----|:-----|
| ChromaDB 规模上限 | 共用 Collection + Metadata 隔离适用于 < 5 万 chunk 总量 | total_chunks > 5 万或 P95 > 500ms 时评估独立 Collection |
