...
| 指标 | 目标值 | 说明 | 来源 |
|:---|:---|:---|:---|
| 检索 Recall@5 | ≥ 0.85 | 向量+BM25 RRF 融合后的 top-5 召回率 | ROADMAP §7.1 / TESTING.md §5 |
| 检索 MRR | ≥ 0.70 | 平均倒数排名 | TESTING.md §5 |
| 检索 Precision@5 | ≥ 0.60 | top-5 精确率 | TESTING.md §5 |
| 端到端 P50 延迟 | ≤ 3s | 50% 的问答在 3 秒内完成 | ROADMAP §7.1 / TESTING.md §8 |
| 端到端 P99 延迟 | ≤ 10s | 99% 的问答在 10 秒内完成 | TESTING.md §8 |
| 答案综合评分 | ≥ 4.0/5.0 | 准确性(40%)+完整性(30%)+溯源(20%)+表达(10%) | TESTING.md §6 |
| 文档入库成功率 | ≥ 99% | 不含格式不支持的预期失败 | PRD 原始需求 |
| 并发用户数 | ≥ 10 | 峰值负载下系统不降级 | TESTING.md §8 |
| 回归测试通过率 | 100% | 每次提交运行全量回归 | TESTING.md §7 |
