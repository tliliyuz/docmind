...
| 检索 MRR | ≥ 0.70 | 压测待补充 | 平均倒数排名（压测时补充） | tests/TESTING.md §5 |
| 检索 Precision@5 | ≥ 0.60 | 压测待补充 | top-5 精确率（压测时补充） | tests/TESTING.md §5 |
| 端到端 P50 延迟 | ≤ 3s | 压测待补充 | 50% 的问答在 3 秒内完成（压测时补充） | ROADMAP §7.1 / tests/TESTING.md §7.3 |
| 端到端 P99 延迟 | ≤ 10s | 压测待补充 | 99% 的问答在 10 秒内完成（压测时补充） | tests/TESTING.md §7.3 |
| 答案综合评分 | ≥ 4.0/5.0 | **第 1 轮 4.38** / **第 2 轮 4.76**（Session 综合分，轮次均分 4.62） / **第 3 轮 4.71** | 准确性(40%)+完整性(30%)+溯源(20%)+表达(10%)，三轮均达标 | ROADMAP §5.5 / §6.6 |
| 文档入库成功率 | ≥ 99% | ✅ 已实现（部分容错分级：<20% warning / 20-50% partial / >50% failed） | 不含格式不支持的预期失败 | PRD 原始需求 |
| 并发用户数 | ≥ 10 | 压测待补充 | 峰值负载下系统不降级（Phase 5 压测验证） | tests/TESTING.md §7.3 |
