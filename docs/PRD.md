...
| 最后更新 | 2026-06-09 |
| 作者 | yuz |
| 状态 | 进行中（Phase 1-4 已完成，Phase 5 设计阶段） |
...
|:---|:---|:---|:---|:---|
| 检索 Recall@5 | ≥ 0.85 | **1.000**（28/28 完全召回） | 向量+BM25 RRF 融合后的 top-5 召回率 | ROADMAP §5.5 / TESTING.md §5 |
| 检索 MRR | ≥ 0.70 | — | 平均倒数排名（压测时补充） | TESTING.md §5 |
| 检索 Precision@5 | ≥ 0.60 | — | top-5 精确率（压测时补充） | TESTING.md §5 |
| 端到端 P50 延迟 | ≤ 3s | — | 50% 的问答在 3 秒内完成（压测时补充） | ROADMAP §7.1 / TESTING.md §7.3 |
| 端到端 P99 延迟 | ≤ 10s | — | 99% 的问答在 10 秒内完成（压测时补充） | TESTING.md §7.3 |
...
| 并发用户数 | ≥ 10 | — | 峰值负载下系统不降级（Phase 5 压测验证） | TESTING.md §7.3 |
