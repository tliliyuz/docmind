...
| 最后更新 | 2026-06-09 |
| 作者 | yuz |
| 状态 | 进行中（Phase 5 设计阶段 — 测试计划细化完成，用例已扩展） |
...
> Phase 5 实现：Locust 压测脚本 `backend/tests/locustfile.py`，模拟真实用户的问答行为（含等待间隔）。4 场景：基准（1并发）/ 日常（5并发）/ 峰值（10并发）/ 极限（20并发）。压测完成后根据 P50/P99 数据设定限流阈值。
